...
pub fn export_vault(vault: &Vault, audit_path: &str, output_path: &str) -> Result<(), String> {
    ...
    let mut tar_data = Vec::new();
    {
        ...
        header.set_mode(0o644);
        builder
            .append_data(&mut header, "vault.json", accounts_json.as_bytes())
            .map_err(|e| format!("Tar error: {}", e))?;
        ...
        if !audit_content.is_empty() {
            ...
            header.set_mode(0o644);
            builder
                .append_data(&mut header, "audit.log", audit_content.as_bytes())
                .map_err(|e| format!("Tar error: {}", e))?;
        }
    ...
    fs::write(output_path, &output).map_err(|e| format!("Cannot write backup: {}", e))?;
...
pub fn import_vault(vault: &Vault, audit_path: &str, input_path: &str) -> Result<usize, String> {
    let data = fs::read(input_path).map_err(|e| format!("Cannot read backup: {}", e))?;
...
mod tests {
    ...
    fn test_export_import() {
        ...
        vault
            .create_account("github", "user", "pass", None)
            .unwrap();
        vault
